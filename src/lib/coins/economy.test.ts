...
describe("coin economy — catalog coverage & stable prices", () => {
  ...
  it("estimates provider costs without changing the owner-set coin scale", () => {
    for (const model of MODEL_CATALOG) {
      const normal = estimatedFighterCostUsd(model.id);
      const deep = estimatedFighterCostUsd(model.id, { deepDebate: true });
    ...
    const input = { modelAId: "gpt-6-astra", modelBId: "deepseek-v4-flash", deepDebate: false, responseLength: "short" as const };
    expect(estimatedMatchCostUsd(input)).toBeGreaterThan(0);
    expect(matchCoinCost(input)).toBe(21);
  });
  ...
  it("anchors match the owner-approved bands", () => {
    ...
    expect(coinPriceForModel("deepseek-v4-pro")).toBe(1);
    expect(coinPriceForModel("z-ai/glm-5.2")).toBe(1);
    expect(coinPriceForModel("gpt-6-astra")).toBe(20);
    expect(coinPriceForModel("deepseek-v4-flash-vision-exp")).toBe(1);
