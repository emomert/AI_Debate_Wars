...
describe("buildMatchShareText", () => {
  it("carries topic, sides, every fighter turn, the verdict and the link", () => {
    const text = buildMatchShareText(
      baseSession(),
      "GPT-5.6 beat Grok 4.5",
      "DeepSeek Chat",
      t,
      "https://debator.app/s?d=abc",
    );
    expect(text).toContain("GPT-5.6 beat Grok 4.5");
    expect(text).toContain("Topic: Should cities ban cars from their centers?");
    expect(text).toContain("For: GPT-5.6 (A)");
    expect(text).toContain("Against: Grok 4.5 (B)");
    ...
    expect(text).toContain("https://debator.app/s?d=abc");
  });
  ...
  it("handles a verdict-less session and no URL", () => {
    ...
    expect(text).not.toContain("See the verdict:");
  });
});
