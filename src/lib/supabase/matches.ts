...
export interface ProfileStats {
  /** Saved matches — all of which are COMPLETED (the saver only writes a match
   *  once its status is "complete"), so this doubles as the completed count. */
  total: number;
  totalCost: number;
  debates: number;
  discussions: number;
  deep: number;
  /** Total rounds across all matches (a gamified engagement total). */
  totalRounds: number;
  /** Distinct fighter models the user has put in the ring. */
...
export function computeStats(rows: MatchSummary[]): ProfileStats {
  ...
  for (const r of rows) {
    ...
    stats.totalRounds += Number(r.round_count) || 0;
  ...
  stats.uniqueFighters = Object.keys(appearances).length;
  stats.topFighter =
