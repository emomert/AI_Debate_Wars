...
Preserve existing model coin prices, pack values, daily eligibility and free Auto judging unless the user explicitly requests a pricing change. Adding models or fixing accounting must not reprice the existing catalog. Cost estimates are informational; do not enforce a margin target by changing customer prices. The September 8 audit uplift was rejected and reverted to the original 1/2/4/8/12/20 bands and Deep Debate +2.
...
- Refreshed 19 existing OpenRouter price pairs and DeepSeek peak rates. Customer coin prices were restored after the owner rejected the audit uplift; provider cost estimates stay separate. See `docs/23_COINS.md`.
...
- Two fighters from ~90 paid models across OpenAI (direct, 14), DeepSeek (direct, 2), and OpenRouter (74 — Claude, Grok, Gemini, Xiaomi MiMo, GLM, Kimi, Nemotron, Qwen, MiniMax, Llama, Mistral, Nova, Hunyuan). **The catalog GROWS, it never rotates**: older generations stay selectable alongside new flagships (users pick), and new models are added ALONGSIDE the previous shortlist entries, never in place of them. Excluded on purpose: `:free` tiers, `-fast`/`-turbo` twins, dated re-releases of an identical checkpoint, parameter-count variants a user cannot choose between, and sub-8B models (they lose the thread over 3 rounds).
